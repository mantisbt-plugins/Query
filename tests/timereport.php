...
if (!function_exists('GetCustomField')) {
    function GetCustomField($bugid) {
        $query = "SELECT * FROM mantis_custom_field_string_table WHERE bug_id = $bugid AND field_id=6";
        $q = db_query ($query);
        $r = db_fetch_array($q);
        return $r['value'];
    }
...
if (!function_exists('CleanStr')) {
    function CleanStr($str) {
        $invalid_characters = array("$", "%", "#", "<", ">", "|", "&amp", "&lt", "br&gt", "/", ";");
        $nstr = str_replace(config_get('plugin_Query_separator', 'Query'), " ", $str);
        $nstr = str_replace("\n", " ", $nstr);
        $nstr = str_replace("\r", " ", $nstr);
        $nstr = str_replace($invalid_characters, "", $nstr);
        return htmlspecialchars($nstr);
    }
}

if (!function_exists('GetResolvedDate')) {
    function GetResolvedDate($bugid) {
        $query = "SELECT FROM_UNIXTIME(date_modified) as resdate FROM mantis_bug_history_table WHERE bug_id=$bugid AND field_name='status' AND new_value=80";
        $q = db_query ($query);
        $r = db_fetch_array($q);
        return $r['resdate'];
    }
...
if (!function_exists('getRelation')) {
    function getRelation($bugid) {
        $query = "SELECT * FROM mantis_plugin_SapData_connect_table as con JOIN mantis_plugin_SapData_relations_table as rel ON rel.cust_id=con.sap_rel_id WHERE con.sap_bug_id = $bugid";
        $q = db_query ($query);
        $r = db_fetch_array($q);
        return $r['cust_name'];
    }
...
if (!function_exists('GetResolvedDateTs')) {
    function GetResolvedDateTs($bugid) {
        $query = "SELECT date_modified FROM mantis_bug_history_table WHERE bug_id=$bugid AND field_name='status' AND new_value=80";
        $q = db_query ($query);
        $r = db_fetch_array($q);
...
if (!function_exists('ExpiredDays')) {
    function ExpiredDays($task_id) {
        $query = "SELECT DATEDIFF(task_completed,task_created) as difference FROM mantis_plugin_Tasks_defined_table WHERE task_id=$task_id";
        $diff = db_query ($query);
...
if (!function_exists('Crea')) {
    function Crea($task_id) {
        $query = "SELECT task_created FROM mantis_plugin_Tasks_defined_table WHERE task_id=$task_id";
        $date = db_query ($query);
...
if (!function_exists('Clo')) {
    function Clo($task_id) {
        $query = "SELECT task_completed FROM mantis_plugin_Tasks_defined_table WHERE task_id=$task_id";
        $date = db_query ($query);
        $r = db_fetch_array($date);
...
$filename = 'TimeReport_'.date(YmdHis) . '.xlsx';
